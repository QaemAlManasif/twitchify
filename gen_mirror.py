...
def icue_name(name):
    """Namespace every mirrored property as tw<Name>.

    iCUE injects one global `let` per meta parameter into the same scope
    main.js runs in, so a property sharing a name with any top-level
    declaration there is a SyntaxError that kills the whole script — the
    widget renders but nothing works. `alertKeywords` and `alertSound`
    did exactly that. Prefixing removes the whole class of collision, and
    check_collisions() below refuses to generate if one ever sneaks back.
    """
    if len(name) > 2 and name.startswith("tw") and name[2].isupper():
        return name
    return "tw" + name[0].upper() + name[1:]


def check_collisions(names):
    """Refuse to generate if a property name would collide."""
    src = io.open("twitch-widget/scripts/main.js", encoding="utf-8").read()
    top = set(re.findall(r"^(?:async\s+)?function\s+([A-Za-z_$][\w$]*)", src, re.M))
    top |= set(re.findall(r"^(?:const|let|var|class)\s+([A-Za-z_$][\w$]*)", src, re.M))
    clash = sorted(set(names) & top)
    if clash:
        raise SystemExit(
            "ABORT — these iCUE property names collide with top-level declarations in "
            "main.js, which is a fatal SyntaxError inside iCUE: " + ", ".join(clash))
...
def main():
    ...
    # Namespace every generated name, then prove none of them collide.
    for g in G:
        G[g] = [(pref, icue_name(name), kind, default, label, extra)
                for pref, name, kind, default, label, extra in G[g]]
    every = [it[1] for items in G.values() for it in items]
    # The hand-written properties in index.html share the same scope.
    check_collisions(every + ["widgetMode", "panelUrl", "textScale", "fsChat",
                              "fsChannels", "fsControls", "fsDash", "followIcueColors"])
