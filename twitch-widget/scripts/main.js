...
// Demo: a believable, fully offline stream, used only to take the
// marketing screenshots. Nothing reaches Twitch.
//
// This is deliberately opt-in by URL and nothing else. It was once also
// triggered by iCUE.isPreview, an undocumented flag guessed at rather
// than looked up — and inside iCUE that turned a placed widget into fake
// channels that reported "Connected" while every real call failed. A
// flag whose meaning isn't documented must not decide what the widget
// shows to a signed-in user.
...
function applyPrefs() {
  ...
  applyIcueStyle(st);                // iCUE's own Custom Style, if it is set
...
// ══ iCUE's Custom Style ═══════════════════════════════════════════
// iCUE personalises a placed widget through its own Custom Style panel,
// which fills the four reserved names below. They are only ever read.
//
// Each is applied only once it has moved off its declared default, so an
// untouched Custom Style leaves the widget's own Appearance preferences
// alone instead of silently overriding them with iCUE's defaults.
const ICUE_STYLE_DEFAULT = {
  accentColor: "#9146ff",
...
function applyIcueStyle(st) {
  const accent = icueColor("accentColor");
  if (accent && accent !== ICUE_STYLE_DEFAULT.accentColor) st.setProperty("--accent", accent);
  const bg = icueColor("backgroundColor");
  if (bg && bg !== ICUE_STYLE_DEFAULT.backgroundColor) st.setProperty("--bg", bg);
  const text = icueColor("textColor");
  if (text && text !== ICUE_STYLE_DEFAULT.textColor) st.setProperty("--text", text);
  const clear = Number(icueProp("transparency"));
  if (Number.isFinite(clear) && clear !== ICUE_STYLE_DEFAULT.transparency) {
    st.setProperty("--stage-alpha", String(1 - Math.min(90, Math.max(0, clear)) / 100));
  }
}

function onIcueData() {
  // Type, panel link, text sizes and Custom Style are all read live.
  applyPrefs();
  if (!$("prefs").hidden) renderPrefs();
}
